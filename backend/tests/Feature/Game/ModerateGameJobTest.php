...
class ModerateGameJobTest extends TestCase
{
    ...
    public function test_handle_approves_game_when_moderation_passes(): void
    {
        ...
        $game->refresh();
        $this->assertSame(GameStatus::Approved, $game->status);
    ...
    public function test_handle_rejects_game_when_moderation_fails_criteria(): void
    {
        ...
        $game->refresh();
        $this->assertSame(GameStatus::Rejected, $game->status);
    ...
    {
        $game = Game::factory()->create(['status' => GameStatus::Pending, 'moderation_attempts' => 0]);
