...
- **Moderação por IA (Gemini)**: `ModerateGameJob` (assíncrono, com retry/backoff) avalia texto + imagem de cada jogo cadastrado e aprova/rejeita automaticamente. Comentários passam por moderação **síncrona** (texto), com fallback configurável em `config/moderation.php` (`MODERATION_COMMENT_FALLBACK`: `reject` = fail-closed/padrão, `approve` = fail-open). Sem `GEMINI_API_KEY`, jogos ficam `pending` indefinidamente e comentários são recusados. Contrato de moderação abstraído por `ModerationServiceInterface` (implementação: `GeminiModerationService`) — trocar de provedor de IA implica nova implementação dessa interface, não espalhar chamadas Gemini pelo código.
